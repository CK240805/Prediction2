...
def train_and_predict(csv_path):
    ...
    return masses_g, final_probs, predicted_set.tolist()

if __name__ == "__main__":
    csv = "toto_results.csv"
    ...
    for idx in top_idx:
        output["top10_overweighted"].append({
            "ball": int(idx+1),
            "probability": round(float(probs[idx]), 5),
    ...
    with open("prediction.md", "w") as f:
        f.write(f"# 🎱 LottOracle Prediction\n\n")
        f.write(f"**Generated:** {now}\n\n")
        f.write(f"**Predicted Set:** {', '.join(f'{n:02d}' for n in pred_set)}\n\n")
        f.write("**Top 10 Over‑Weighted Numbers:**\n")
        for item in output["top10_overweighted"]:
            f.write(f"- Ball {item['ball']:02d}: prob {item['probability']:.4f} (edge {item['edge']:.5f})\n")

    print("Prediction saved to prediction.json and prediction.md")
    print("Predicted set:", pred_set)
